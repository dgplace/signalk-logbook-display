...
Manual voyages can be added from the desktop layout (non tab view) by clicking **Add manual voyage** at the bottom of the table to reveal the entry panel. The table panel expands while the form is open so the full manual voyage form stays visible, and it returns to its previous size when you close the panel. Enter the start/end date and time plus named start/end locations with coordinates (or use the Pick on map buttons to populate lat/lon); if the end date is missing or earlier than the start, it is auto-reset to match the start. The UI calculates distance and an estimated duration based on a 6 kn average speed, stores the record separately, and merges it into the table chronologically. Manual voyages render in gray on the map and suppress Max Speed, Max Wind, Avg Wind, and Wind Dir values in the table. The location name list is remembered so future entries can re-use saved coordinates. Manual voyages persist in `public/manual-voyages.json` and can be edited via the Edit button in the rightmost column; the panel includes a Delete manual voyage button while editing. The manual entry panel and edit/delete controls are hidden in the mobile tab layout.
