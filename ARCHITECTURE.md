...
- Summary panel Travel Time is computed by summing voyage total hours (leg durations for parsed voyages and distance-based hours for manual entries); Sea Time is computed from voyage start/end timestamps; Night Time is computed from sailing/motoring point gaps in generated voyages only that occur between civil dusk and dawn using a local solar day boundary; average/max speed are computed across generated voyages only; Sailing Time still derives from point activity samples and is capped to Travel Time.
