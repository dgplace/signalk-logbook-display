...
- Front-end single-page app (SPA) renders voyage tracks on Leaflet maps, provides rich voyage analytics, includes responsive layout controls for the map/table tabs on mobile, and shows leg total-time hours (h) in cell values plus voyage totals (including summed multi-leg durations) while surfacing Travel Time, Sea Time, Night Time, total distance (NM with km in parentheses), Sailing Time, plus overall/sailing/motoring average speeds across generated voyages in a summary panel and averaging multi-leg voyage speeds from leg averages.
...
2. Manual voyage entries are stored in `public/manual-voyages.json` (including ordered `locations` arrays for multi-leg trips with optional per-leg `routePoints`, plus day-trip `routePoints`/`routeTurnIndex`) and loaded as a static JSON asset alongside `voyages.json`, with the `/manual-voyages` API used for create/update/delete operations.
...
6. `public/app.js` fetches the JSON resources, merges generated and manual voyages chronologically, derives leg segments from anchored activity or skip-connection markers (allowing manual legs under 1 nm), computes leg durations to drive total-time/average-speed display (including summed multi-leg totals) and summary-panel Travel Time/Sea Time/Night Time/Distance/Sailing Time totals, renders voyages on the map, and exposes UI interactions.
...
- **public/view.js**: Responsive layout utility that manages mobile/desktop layout switching, tab behaviour, and layout-driven map resizing.
...
- **public/gpx.js**: Utility module that generates GPX XML from voyage points (including per-point extension metadata) and triggers browser file downloads.
...
- Manual voyage entries are stored in `public/manual-voyages.json`, loaded alongside other JSON assets, and surfaced through a movable overlay panel on the map.
- Manual voyage entry is toggled by the Add manual voyage button, supports tabbed Start/To entries with "Add Anchorage" to insert intermediate stops (labeled "Anchorage N"), map clicks to populate coordinates, includes a top-level "Day trip (return to start)" checkbox for round trips that doubles distance/time and prevents adding anchorages, optional day-trip loop editing on the map with insert/move/remove points and direction arrows, per-leg route editing for multi-stop voyages with open A→B segment support (custom waypoints stored in each location's `routePoints` array and reused for saved track/highlight rendering), auto-fills stop times from the previous stop plus leg duration (deriving a read-only final stop time), manual durations assume 5 kn average speed (using custom route distances when available), and manual rows expose an Edit button for in-place updates via the panel.
- During both creation and editing, the manual voyage panel provides real-time map feedback: clicking Edit automatically selects and focuses the voyage on the map, the currently active stop is highlighted with a pulsing orange marker (including stop name in tooltip), and a dashed gray preview track connects all valid stops with arrows showing day-trip loop direction when enabled.
...
- Event coordination: `MANUAL_LOCATION_SELECTED` event triggers `highlightLocation()` for stop markers, `MANUAL_VOYAGE_PREVIEW` event triggers `drawManualVoyagePreview()` for the preview track, and `MANUAL_ROUTE_EDIT_REQUESTED`/`MANUAL_ROUTE_UPDATED` keep day-trip loop edits in sync between manual.js and map.js.
- Manual voyage map tracks render in gray and hide max speed, max wind, avg wind, and wind direction cells in the table.
- Opening the manual voyage panel temporarily hides the Point Details panel to reduce map clutter; closing it restores the details panel.
...
- Summary panel Travel Time is computed by summing voyage total hours (leg durations for parsed voyages and distance-based hours for manual entries); Sea Time is computed from voyage start/end timestamps; Night Time is computed from sailing/motoring point gaps in generated voyages only that occur between civil dusk and dawn using a local solar day boundary; average speed values (overall/sailing/motoring) are computed across generated voyages only; Sailing Time still derives from point activity samples and is capped to Travel Time.
